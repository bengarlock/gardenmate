...
export function ResourceTrackerPage({
    title = 'Resources',
    eyebrow = 'Chickens',
    eyebrowHref = '/chickens',
    subNav = <ChickenSubNav />,
    trackerApi = RESOURCE_TRACKER_API,
    itemFilter = () => true,
    payloadTransform = (payload) => payload,
    notesForDisplay = (notes) => notes,
    activeLabel = 'tracked',
    ...
    const [pausingItemId, setPausingItemId] = useState(null)
    ...
    async function loadItems() {
        ...
        setLoading(true)
        try {
            const response = await fetch(trackerApi, { cache: 'no-store' })
    ...
    async function createItem(event) {
        ...
        setSaving(true)
        setError('')
        const payload = payloadTransform({
            name: form.name.trim(),
        ...
        try {
            const itemUrl = editingItemId ? `${trackerApi}/${editingItemId}` : trackerApi
            ...
            if (editingItemId) {
                setItems((current) => {
                    if (!itemFilter(data)) return current.filter((entry) => entry.id !== data.id)
    ...
    async function resetItem(item) {
        ...
        try {
            const response = await fetch(`${trackerApi}/${item.id}/reset`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({}),
            })
    ...
    async function archiveItem(item) {
        setError('')
        const response = await fetch(`${trackerApi}/${item.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function togglePauseItem(item) {
        setError('')
        setPausingItemId(item.id)
        ...
        try {
            ...
            const data = await response.json().catch(() => ({}))
            if (!response.ok) {
                setError(data.message || `Could not ${nextStatus === 'paused' ? 'pause' : 'resume'} resource.`)
                return
            }
            setItems((current) => current.map((entry) => (entry.id === item.id ? data : entry)))
        } finally {
            setPausingItemId(null)
        }
    }
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm text-stone-300">
                                {loading ? 'Loading' : `${sortedItems.length} ${activeLabel}`}
                            </p>
                        </div>
                        <button
                            type="button"
                        ...
                        >
                            <div className="grid gap-4">
                                <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-3">
                                    <h3 className="text-lg font-semibold text-white">{createTitle}</h3>
                                    <button
                                        type="button"
                                        onClick={closeForm}
                                            ...
                                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                                <div className="min-w-0">
                                                    ...
                                                    <h2 className="mt-1 break-words text-xl font-semibold text-white">
                                                        <span className="inline-flex flex-wrap items-center gap-2">
                                                            ...
                                                            {item.name}
                                                            {isPaused ? (
                                                                <span className="rounded-full border border-amber-200/50 px-2 py-0.5 text-xs font-semibold uppercase text-amber-100">
                                                                    Paused
                                                                </span>
                                                            ) : null}
                                                        </span>
                                                    ...
                                                    <p className="mt-1 text-sm text-stone-300">
                                                        Last reset {formatDate(item.last_reset_at)} ({formatDaysSince(item.last_reset_at)}). {isPaused ? `Paused since ${formatDate(item.paused_at)}.` : `Due ${formatDate(item.next_depletion_at)}.`}
                                                    </p>
                                                </div>
                                                <div className="flex shrink-0 flex-wrap gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={pausingItemId === item.id}
                                                        className={`min-h-10 rounded-md border px-3 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${
                                                            isPaused
                                                                ? 'border-emerald-300/40 text-emerald-100 hover:bg-emerald-400/10'
                                                                : 'border-amber-200/40 text-amber-100 hover:bg-amber-400/10'
                                                        }`}
                                                    >
                                                        {pausingItemId === item.id ? 'Saving' : isPaused ? 'Resume' : 'Pause'}
                                                    </button>
                                                    <button
                                                        type="button"
                                                ...
                                                >
                                                    <div className="grid gap-4">
                                                        <div className="flex items-center justify-between gap-3">
                                                            <h3 className="text-base font-semibold text-white">{editTitle}</h3>
                                                            <button
                                                                type="button"
                                                                onClick={closeForm}
                                                                className="min-h-10 rounded-md border border-white/15 px-3 text-sm font-semibold text-stone-100 transition hover:bg-white/10"
